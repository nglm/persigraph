...
class PersistentGraph():
    ...
    def __init__(
        ...
        self.__k = np.ones((self.T,), dtype=int)
        self.__current_probas = np.ones((self.T,))/self.N
        self.__current_entropy = entropy()  # Current entropy at each time step
        self.__future_cluster = None        # Label predictions if k=k+1
        self.__future_entropy = None        # Entropy if we create another node
