...
# type: str
path_fig = "/home/natacha/Documents/tmp/figs/pairplots_first_location_compare_t2m_tcvw_with_log/"
...
for filename in list_filenames:
    ...
    if use_log_tcwv:
        ...
        if idx != -1:
            for i in idx:
                list_var.append(np.log(list_var[i]))
                list_names.append("log(tcwv)")
    ...
    if use_standardise:
        fig_suptitle = (
            "Bergen Forecast: "
            + filename[:-3]
            + "\n First grid points, All members, standardized values"
        )
    else:
        fig_suptitle = (
            "Bergen Forecast: "
