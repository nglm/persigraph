...
def talk_figs():
    ...
    min_dates = [
        None, # Lothar
        None, # Sandy
        datetime.datetime(2019,  7, 25, 0), # Heatwave
        None, # Coldwave
    ]
    names = ['Lothar', 'Sandy', 'heatwave', 'coldwave']
    idx = [
    ...
    min_t = [0]*len(min_dates)
    # Late starting date for the heawave
    min_t[2] = list(data[names[2]]['dates'][idx[2]]).index(min_dates[2])
    ...
    # ----- Prepare folders and paths ------------
    # --------------------------------------------
    path_root = PATH_FIG_PARENT + 'best_talk/'
    path_fig = path_root + "plots/"
    path_graph = path_root + "graphs/"
    makedirs(path_fig, exist_ok = True)
    makedirs(path_graph, exist_ok = True)
    ...
    def lothar():
        ...
        _, axk, _ = k_plot(g, k_max = 5, ax=axk, show_legend=False)
        axk.set_ylabel("Life span")
        axk.set_xlabel("")
        axk = use_dates_as_xticks(ax1,  d['time'][i][:max_t[k]], freq=8)
    ...
    def heatwave():
        ...
        #common_t = find_common_dates(d['time'][i][:max_t[k]], d['obs_time'])
        print(min_t, max_t)
        print(d['var'][i][:,min_t[k]:max_t[k]].shape)
        common_t = find_common_dates(
            d['time'][i][min_t[k]:max_t[k]],
            d['obs_time'])
        # Add control member (because otherwise we had the special 50%-50% prop)
        members = np.concatenate(
            [d['var'][i][:,min_t[k]:max_t[k]],
            d['ctrl_var'][i][min_t[k]:max_t[k]].reshape(1,-1)]
        ) - 273.15
        ...
        # # ---- k_plot ----
        ax1 = fig.add_subplot(gs[19:27, (m-n_box)//2+2:(m-n_box)//2+14])
        _, ax1, _ = k_plot(g, k_max = 5, ax=ax1, show_legend=False,)
        ax1.set_ylabel("Life span")
        ax1.set_xlabel("")
        #title = 'Number of modes: relevance'
        ...
        text_kw = {"fontsize" : 12}
        ax1 = use_dates_as_xticks(
            ax1,  d['time'][i][min_t[k]:max_t[k]], freq=8,
            text_kw=text_kw
            )
        ...
        ax2 = add_annotation(
            ax2, 'A',
            xloc = 0.5, yloc = 0.95, fontsize = 27,
        )
        ax0.plot([d['time'][i][26]]*2, [10,14.5], lw=lw_date, c='black')
        ax0.plot([d['time'][i][26]]*2, [29,31.5], lw=lw_date, c='black')
        ax0 = add_annotation(
        ...
        ax2.set_ylabel('Temperature (°C)', fontsize=18)
        ax2.tick_params(axis='y', which='major', labelsize=18)
        ax2 = use_dates_as_xticks(ax2,  d['time'][i][min_t[k]:max_t[k]])
        # Remove first xtick because of overlapping
        ax2.set_xticks(ax2.get_xticks()[:-1])
        # We can not really share without that if they have been created
        # separately
        ...
        for show_mean in [True, False]:
            ...
            axs = use_dates_as_xticks(axs,  d['time'][i][min_t[k]:max_t[k]])
            axs.set_ylabel('Temperature (°C)', fontsize=18)
            axs = add_ctrl(
                    ctrl_var=d['ctrl_var'][i][min_t[k]:max_t[k]]-273.15,
                    dates=d['time'][i][min_t[k]:max_t[k]],
                    ax=axs
                )
            ...
            if show_mean:
                name_fig = (
                    path_fig + name +'_'
                    + filename[:-3] + "_" + d['var_name'] +"_with_mean"
                )
        ...
        # ---------------------------
        # meteogram with bimodality emphasized
        # ---------------------------
        idx_date_min = 26-16-2
        idx_date_max = 26-16+2
        for show_mean in [True, False]:
            ...
            for m in members:
                # Before
                axs.plot(
                    d['time'][i][min_t[k]:min_t[k]+idx_date_min+1],
                    m[:idx_date_min+1],
                    lw=LW, color=COLOR)
                # After
                axs.plot(
                    d['time'][i][min_t[k]+idx_date_max:max_t[k]],
                    m[idx_date_max:],
                    lw=LW, color=COLOR)
            # Dates of interest
            idx_members_up = [
                i for i, m in enumerate(members) if m[26-16]>22]
            idx_members_down = [
                i for i, m in enumerate(members) if m[26-16]<=22]

            idx_members = [idx_members_up, idx_members_down]
            members_colors = ['green', 'blue']
            labels = ['First mode', "Second mode"]
            for i_mode in range(2):
                for i_m, m in enumerate(members[idx_members[i_mode]]):
                    if i_m == 0:
                        axs.plot(
                            d['time'][i][
                                min_t[k]+idx_date_min:min_t[k]+idx_date_max+1
                            ],
                            m[idx_date_min:idx_date_max+1],
                            lw=LW, color=members_colors[i_mode],
                            label=labels[i_mode])
                    else:
                        axs.plot(
                            d['time'][i][
            ...
            if show_mean:
                name_fig = (
                    path_fig + name +'_'
                    + filename[:-3] + "_" + d['var_name'] +"_colored"
                )
